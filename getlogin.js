(function() {
    let ready = false, timeoutclean = 0
    function add_login() {
        ready = true
        ...
        __e.b.addEventListener('click', function() {
            timeoutclean = setTimeout(function() { __e.p.value = '' }, 6e4)
            ...
            if (isNaN(g) || gl > 7 || gl < 5) {
                ...
                let strSum = _k.substring(v.i, v.f), _y = '', _h = ''
        ...
        __e.p.addEventListener('keydown',function(v) {
            if (v.code == 'Enter' && v.shiftKey && !v.ctrlKey) {
                __e.b.click()
            }
        })
        window.document.addEventListener('keydown',function(v) {
            if (v.shiftKey && !v.ctrlKey && v.code == 'KeyI') {
                alert(v_r + 1)
                __e.p.focus()
                setTimeout(function() {
                    __e.p.value = ''
            ...
            if (v.shiftKey && v.code == 'Backspace') { cl() }
        })
        __e.b.addEventListener('dblclick', function() { cl() })
    }
    function checkAddReady() {if(!ready){add_login()}}
    window.addEventListener('load', function() {checkAddReady()})
    setTimeout(function() {checkAddReady()}, 1e3)
    setTimeout(function() {checkAddReady()}, 5e3)
})()